...
- **Testing was prioritized as the top gap** and has been substantially closed (249 tests now exist, including `TestRoleTiers` for the RBAC split, `TestDatabaseSchemaChange` for the portability work, and `TestCatalogApprovalWorkflow` for the catalog registry).
- **A prioritized backlog of five further improvements is done**: `app.py` refactored from a ~1,200-line monolith into service-oriented modules (`routes/`, `query_service.py`, `llm_client.py`, `prompt_builder.py`, `sql_safety.py`, `auth_decorators.py`); structured logging + a real "System health" dashboard in the admin panel (previously the analytics data existed via API but was surfaced nowhere); a hermetic benchmark script (`benchmarks/run_benchmarks.py`) with real p50/p95 latency numbers per resolution path; the YAML catalog replaced with a versioned, SQLite-backed registry requiring a separate approval step before a promoted entry answers live traffic; and deepened LLM-prompt dialect coverage (Postgres, MSSQL, alongside the existing SQLite/MySQL).
- Still deferred (per explicit instruction not to duplicate existing coverage, and not yet requested): a full audit-trail/trace system beyond `query_analytics.py`'s per-request log, and query-explanation generation for end users.
